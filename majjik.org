...
***** TODO COMMENT sectioned mutable process log
****** proof of concept
#+begin_src elisp
  ;; -*- lexical-binding: t; -*-
  (let ((buf (generate-new-buffer "log"))
        (val `(:log))
        (sec-parts))
    (with-current-buffer buf
      (magit-insert-section (root)
        (magit-insert-section sec
          (foo val)
          (let ((header-regions
                 (xo/insert-sectioned-heading
                  '("["
                    (:code . "   ")
                    "]> jj "
                    (:args . "<args >")
                    "(cmd)\n"))))
            (setq sec-parts `(,sec ,@header-regions)))))
      (jj-inspect-mode)
      (let* ((sec (car sec-parts))
             (parts (cdr sec-parts))
             (code (alist-get :code (dbg parts)))
             (args (alist-get :args parts))
             (buf-code (with-current-buffer (clone-indirect-buffer "code-buf" nil)
                         (narrow-to-region (car code) (cdr code))
                         (current-buffer)))
             (ovl-args (make-overlay (car args) (cdr args)))
             (fn (let ((count -1))
                   (lambda ()
                     (with-current-buffer buf
                       (xo/modify-section-header sec
                         (lambda ()
                           (with-current-buffer buf-code
                             (let ((str (buffer-string))
                               ...
                               (cl-incf count)))))
                       (xo/modify-section-body sec
                         (lambda ()
                           (goto-char (point-max))
                           (insert (format "foo %d\n" count)))))))))
        (push ovl-args (cdr val))
        (run-at-time "1 sec" nil fn)
        (run-at-time "2 sec" nil fn)
        (run-at-time "3 sec" nil fn)
        (run-at-time "4 sec" nil fn)
        (run-at-time "5 sec" nil fn)
        (run-at-time "6 sec" nil fn)))

    (pop-to-buffer buf))

  (define-advice magit-section-show (:after (sec) xo/magit-section-show-args)
    (pcase (oref sec value)
      (`(:log ,ovl-args)
       (overlay-put ovl-args 'display nil))))

  (define-advice magit-section-hide (:after (sec) xo/magit-section-hide-args)
    (pcase (oref sec value)
      (`(:log ,ovl-args)
       (overlay-put ovl-args 'display ""))))

  (defun xo/modify-section-header (sec fun)
    ...
    (save-excursion
      (with-restriction (oref sec start) (oref sec content)
        (let ((inhibit-read-only t))
          ...
          (xo/propertize-buffer-for-section sec)))))

  (defun xo/modify-section-body (sec fun)
  ...
  (defun xo/propertize-buffer-for-section (sec)
  ...
  (defun xo/insert-sectioned-heading (header-parts)
  ...
  corresponding labeled string starts and ends."
    (xo/insert-sectioned header-parts
                         #'magit-insert-heading))


#+end_src
****** TODO draft replacement
#+begin_src elisp
  (defun jj--make-process-log-entry (name cmd &optional min-cmd hide-args)
    "Return a `jj--process-log-entry' for indirectly writing to the jj log buffer for the current repo. CODE contains the process status info, and STDOUT and STDERR the respective streams. If provided, MIN-CMD is a shorter version of CMD, e.g. omitting majjik's default arguments."
    (let ((repo-dir default-directory))
                ...
                `(,@(xo/insert-sectioned-heading
                  ...
                  ,@(xo/insert-sectioned
                     `(:content
                       ...
                       "\n"
                       :end)))
              (let* ((code-buf (xo/make-narrowed-indirect (format "*code-section %s*" display-name) (car code) (cdr code)))
                     (stdout (xo/make-narrowed-indirect (format "*stdout-section %s*" display-name) stdout stdout))
                     (stderr (xo/make-narrowed-indirect (format "*stderr-section %s*" display-name) stderr stderr))
                     ;; these need to be overlays,
                     ;; so we can toggle their properties as a unit
                     (ovl-args (make-overlay (car args) (cdr args)))
                     (ovl-collapse (make-overlay content end))
