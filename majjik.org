...
#+begin_src elisp
  ...
  (let* ((program (jj--editor-path))
         (server (jj--editor-server-path))
         (edit (concat "merge-tools._emacs.program=" program))
         (args `(,server "$left" "$right"))
         (edit-args ;; Tell the process where the server file is.
          (format "merge-tools._emacs.edit-args=[%s]"
                  (s-join ","
                          (mapcar #'prin1-to-string
                                  args)))))
    (with-editor "JJ_EDITOR"
      (jj-cmd-async "split" `("split" "-r" "@" "-i"
                              "--config" ,edit
                              "--config" ,edit-args
                              "--debug"
                              "--tool" "_emacs"
