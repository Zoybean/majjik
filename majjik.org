...
#+begin_src elisp :comments no
  ;;; majjik.el ends here
#+end_src
* TODO use one buffer for all command output?
find uses of get-buffer-create, replace with somethign that makes a section and an indirect buffer within a common command-log buffer (local to the repo, not emacs-global)
then I could do like magit when there's an error, and direct user to look at the common log.
