...
#+begin_src elisp
  ...
  (defun jj--merge-tool-args (server-edit-function)
    "Returns the arguments needed to register emacs as the merge tool for a JJ command.
  SERVER-EDIT-FUNCTION must be a quoted sexpression that would be a valid argument to `function' (i.e. a lambda or a symbol). The resulting function must be a valid argument to `jj--call-from-cli'.
