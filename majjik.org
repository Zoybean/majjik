...
#+begin_src elisp
  ;; TODOs:
  ;; - last value as init
  ;; - history in the editing buffer. needs:
  ;;   - new function like `read-string-from-buffer' which has:
  ;;     - history
  ;;     - a keymap to cycle history
  ;; - truncating the display of the value. needs:
  ;;   - its own new infix subclass
  ;;   - new method on that class for `transient-format-value'
  ;; - multiple ways to set one arg
  ...
  (defun string-edit-history--maybe-save-current (mode)
    "If the position in the history is 0, save the editable region as the initial element and return the new value for MODE.
  Assumes the current buffer is a string-edit input buffer."
    (-let (((list pos initial) mode))
      (if (= pos 0)
          `(,list ,pos ,(string-edit-history--current))
        mode)))
  ...
  (defun string-edit-history-previous ()
    ...
    (cl-callf string-edit-history--maybe-save-current
  ...
  \\<string-edit-history-mode-map>\
  If HIST-VAR is non-nil, it should be a symbol whose value cell
  is a list. \\[string-edit-history-next] and \\[string-edit-history-previous] can be used to traverse the
  ...
  (transient-define-prefix jj-commit-prefix ()
    ...
    ["meta"
     ("-m" "message" "--message="
      :always-read t
      :reader (lambda (prompt init _hist)
                (read-string-from-buffer prompt (or init ""))))
     ("-D" "dry run" "--dry-run")]
    ["go"
     ("c" "commit" (lambda (args)
                     (interactive (list (transient-args (oref transient-current-prefix command))))
                     (message "jj commit %s" (s-join " " args))))
     ("n" "new" (lambda (args)
                  (interactive (list (transient-args (oref transient-current-prefix command))))
                  (message "jj new %s" (s-join " " args))))
     ("e" "edit" (lambda (args)
                   (interactive (list (transient-args (oref transient-current-prefix command))))
                   (message "jj edit %s" (s-join " " args))))
     ("d" "describe" (lambda (args)
                       (interactive (list (transient-args (oref transient-current-prefix command))))
                       (message "jj describe %s" (s-join " " args))))])
#+end_src
****** move
#+begin_src elisp
