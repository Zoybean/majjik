...
#+begin_src elisp :tangle no
  ...
  (setq default-directory "~/repos/jj-test")

  ;; return a promise that is resolved (returning the process) when the process succeeds
  ;; or rejected (with the process and event) when the process fails.
  (let ((name "foo")
        (cmd '("showa" "-r" "@")))
    (let ((repo-dir default-directory))
  ...
  (defun jj-cmd-promise (name cmd &optional no-revert silent-ok)
    "Run CMD asynchronously, returning a promise of its completion.

  On success, reverts the repo's dash buffer unless NO-REVERT, prints a message unless SILENT-OK, and returns the process. On error, prints a message indicating the command log buffer, and returns a cons (PROCESS . EVENT)."
    (declare (indent 2))
    (promise-then
      ...
      (-let (((code stdout . stderr) (jj--make-process-log-section-buffers name cmd)))
        ...
        (let ((proc (make-process
          ...
          (add-function :after (process-sentinel proc)
                        (jj--make-cleanup-sentinel stderr stdout code))))))))
#+end_src
***** TODO this callback interface sucks
expose an ~error~ parameter, not an ~ok~ parameter.
(see also, make-jj-callback-sentinel, which would have to change)
**** view
***** jj diff
#+begin_src elisp
