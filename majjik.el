    ...
    (cl-loop for x from 1 below jj--count-graph-lines
             ...
             do (progn
                  (jj--re-step-over (rx (group (* (not control)))
                                        (literal jj--major-delim)
                                        "\n"))
                  (push (match-string 1) graph)
                  (delete-region (match-beginning 0) (match-end 0))))
    (apply #'vector (nreverse graph))))
...
(defun jj-parse-erase-elided-prefix ()
  (let ((graph))
    (jj--re-step-over (rx line-start
                          ...
                          "\n"))
    (let-match (pre node post)
      (push `[,pre ,node ,post] graph))
    (delete-region (match-beginning 0) (match-end 3))
    (cl-loop while (looking-at (rx line-start
                                   (* (not control))
                                   line-end
                                   "\n"))
             do (progn
                  (push (match-string 0) graph)
                  (delete-region (match-beginning 0) (match-end 0))))
    (apply #'vector (nreverse graph))))
...
"))
    (should (equal (cl-loop while (not (eobp))
                            for n = (let ((errors))
                                      (or (push-errors errors (cons :entry (with-error-format
                                                                               "error reading log node: %s"
                                                                             (jj-read-graph-and-entry))))
                                          (push-errors errors (cons :elided (with-error-format
