...
(defun jj-list-marked-sections (&optional sec)
  "List all sections in the current buffer that are marked. If SEC is non-nil, only list marked sections among it and its children."
  (let ((marked nil))
    (jj-map-marked-sections
     (lambda (sec)
       (push sec marked))
     sec)
    marked))

(defun jj-map-marked-sections (fn &optional sec)
  "Call FN with each section, in a depth-first post-order traversal. If SEC is provided, traverse it and its children only."
  (magit-map-sections
   (lambda (sec)
     (when (and sec
                (jj-markable-section-p sec)
                (oref sec marked))
       (funcall fn sec)))
   sec))

(defun jj-toggle-mark-section (sec)
  "Toggle whether section SEC is marked. Interactively, the section at point."
  (interactive (list (magit-section-at))) 
  (when (or (and sec
                 (jj-markable-section-p sec))
            (user-error "Not a markable section"))
    (prog1 (oset sec marked (not (oref sec marked)))
      (run-hook-with-args
       'jj-after-section-mark-change-hook
       sec))))

(defun jj-mark-section (sec)
  "Mark section SEC. Interactively, the section at point."
  (interactive (list (magit-section-at))) 
  (when (and (or (and sec
                      (jj-markable-section-p sec))
                 (user-error "Not a markable section"))
             (not (oref sec marked)))
    (prog1 (oset sec marked t)
      (run-hook-with-args
       'jj-after-section-mark-change-hook
       sec))))

(defun jj-unmark-section (sec)
  "Unmark section SEC. Interactively, the section at point."
  (interactive (list (magit-section-at))) 
  (when (and (or (and sec
                      (jj-markable-section-p sec))
                 (user-error "Not a markable section"))
             (oref sec marked))
    (prog1 (oset sec marked nil)
      (run-hook-with-args
       'jj-after-section-mark-change-hook
       sec))))

(defun jj-mark-sections-region (beg end)
...
(defun jj-unmark-sections-region (beg end)
...
(defun jj-mark-dwim ()
  (interactive)
  (if (use-region-p)
      (call-interactively #'jj-mark-sections-region)
    (call-interactively #'jj-mark-section)))
(defun jj-unmark-dwim ()
  (interactive)
  (cond ((use-region-p)
         (call-interactively #'jj-unmark-sections-region))
        (:else
         (call-interactively #'jj-unmark-section))))

(defun jj-unmark-all-sections (&optional sec)
  (interactive)
...
;; [[file:majjik.org::*kludge to highlight marks][kludge to highlight marks:1]]
(defface jj-mark-section-face
  ...
  "Face for marked section headings."
  :group 'magit-section-faces)

(defvar jj-after-section-mark-change-hook ()
  "Hook called after a section is marked or unmarked.")

(defun jj-repaint-mark-section (sec)
  (setq magit-section-highlight-force-update t))

(add-hook 'jj-after-section-mark-change-hook #'jj-repaint-mark-section)


(define-advice magit-section-update-highlight (:override (&optional force) jj/magit-section-update-highlight)
  "Highlight as usual, but also highlight marked sections."
  (let ((section (magit-current-section))
        (focused (magit-focused-sections))
        (marked (jj-list-marked-sections)))
    (cond
     ((or force
          marked
      ...
      (let ((inhibit-read-only t)
        ...
        (dolist (mark marked)
          (with-slots (start content end) mark
            (let ((ov (make-overlay start (or content end) nil t)))
              (overlay-put ov 'font-lock-face 'jj-mark-section-face)
