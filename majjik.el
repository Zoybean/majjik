...
- `:separator' the separator to insert before this field, rather than a space (or empty for the first field). Only inserted if the value is present."
  `(progn
     ...
     (defun jj--insert-log-header (header)
       "Insert the HEADER object formatted for a jj log."
       ,(cl-labels ((field (name-sym)
          ...
          `(progn
             (cl-loop for (name val printer face sep) in (list ,@(cl-loop  
                      ...
                      do (when-let ((printed (s-presence
                                              (if printer
                                                ...
                                                val))))
                           ;; combine the field property
                           ;; (push-text-property 0 (length printed)
                           ;;                     jj-field-path (,name) printed)
             ...
             (unless (bolp)
               (insert "\n")))))
     (defun jj--prefix-insert-log-graph (graph)
       "Insert the GRAPH object as a prefix to all lines of the current buffer."
       ...
       (let ((target-buffer (current-buffer)))
         ...
         (with-temp-buffer
           (let ((header (jj-log-entry-header entry))
             ...
             (jj--insert-log-header header)
             (jj--prefix-insert-log-graph graph)
...
When ABSOLUTE-PATHS, return fully expanded file names. Otherwise, return paths relative to ROOT."
  (let* ((root (file-name-as-directory (expand-file-name root)))
         (untracked-paths (cl-loop with stack = ()
                                   with untracked = ()
                                   with tracked = (mapcar (lambda (trk)
                                                            (expand-file-name trk root))
                                                          tracked-paths)
                                   for dir = root then (pop stack)
                                   for list = (directory-files dir :full
                                                               (rx  (|
                                                                     ;; non-dot covers any normal path that might include a .
                                                                     (not ".")
                                                                     ;; 3 (or more, implicitly) covers any case where a path is only ., but it's still not a relative path
                                                                     (= 3 ".")
                                                                     )))
                                   do (cl-loop for file in list
                                               do (pcase file
...
(defun jj-field-at-point ()
  (get-text-property (point) 'jj-field))
